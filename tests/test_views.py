...
def test_top_operations_basic():
    """Тест базовой функциональности"""
    operations = [
        {'Сумма операции': 100, 'Дата платежа': '2023-01-01', 'Категория': 'Еда', 'Описание': 'Обед'},
        {'Сумма операции': 500, 'Дата платежа': '2023-01-02', 'Категория': 'Транспорт', 'Описание': 'Такси'},
        {'Сумма операции': 300, 'Дата платежа': '2023-01-03', 'Категория': 'Развлечения', 'Описание': 'Кино'},
        {'Сумма операции': 200, 'Дата платежа': '2023-01-04', 'Категория': 'Еда', 'Описание': 'Ужин'},
        {'Сумма операции': 400, 'Дата платежа': '2023-01-05', 'Категория': 'Покупки', 'Описание': 'Одежда'},
        {'Сумма операции': 50, 'Дата платежа': '2023-01-06', 'Категория': 'Еда', 'Описание': 'Кофе'}
    ]

    result = top_operations(operations)

    # Проверяем, что возвращается 5 операций
    assert len(result) == 5

    # Проверяем, что операции отсортированы по убыванию суммы
    assert result[0]['Сумма операции'] == 500
    assert result[1]['Сумма операции'] == 400
    assert result[2]['Сумма операции'] == 300
    assert result[3]['Сумма операции'] == 200
    assert result[4]['Сумма операции'] == 100

    # Проверяем, что остались только нужные ключи
    expected_keys = {'Дата платежа', 'Сумма операции', 'Категория', 'Описание'}
    for operation in result:
        assert set(operation.keys()) == expected_keys
